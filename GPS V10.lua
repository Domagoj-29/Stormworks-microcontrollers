...
local function getCoordinates()
	...
	local estimateOffset = h == 32 and 7 or 0
	return
	{
...
function onTick()
	...
	if insertWaypointPulse and WaypointMode == "Single" then
...
function onDraw()
	...
	if ScreenMode == "Map" then
		...
		for i = 1, 0, -1 do
			...
			elseif PointerType == "Square" then -- Square and reference pointers intentionally don't have shading
	...
	elseif ScreenMode == "Data" then
		for i = 1, h - 2 do
			screen.setColor(20, 20, 20, 255 - i / (h - 2) * 50)
